...
class SIFTHelper:
    def __init__(self,flags_path='./flags'):
        ...
        self.__locs=[]
        self.__uuid=[]
        self.__database_path=os.path.join(flags_path,'sift')
    ...
    def saveSIFT(self,locs,desc,path):
        debug(locs)
        debug(desc)
        np.save(path,np.hstack((locs,desc)))
    ...
    def load_SIFT_from_database(self):
        ...
        debug('finish loading sift features, the first one is')
        #debug(self.__flags[0])
    
    def save_SIFT_to_database(self,id=None,locs=None,sift=None):
        if id is not None:
            self.saveSIFT(locs,sift,os.path.join(self.__database_path,f"{id}.d.txt"))
        else:
            for i in range(0,len(self.__uuid)):
                self.saveSIFT(self.__locs[i],self.__flags[i],os.path.join(self.__database_path,f"{self.__uuid[i]}.d.txt"))
        
    
    def sift_alignment(self,query,train,dist=0.6,fac=5):
        debug("task:")
        debug(query)
        debug(train)
        ...
        return good_matches>fac

    
    def sift_alignment_with_database(self,img,fac=5):
        key,desc=self.getSIFT(img)

        for i in range(0,len(self.__flags)):
            #debug('this sift', self.__flags[i])
            if self.sift_alignment(self.__flags[i],desc):
